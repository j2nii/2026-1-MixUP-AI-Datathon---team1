...
def run_guardrail_agent(sql_query: str) -> dict:
    ...
    guardrail_system = (
        ...
        "[필수 방어 규칙]\n"
        "- 입력된 텍스트가 올바른 SQL 문장(예: SELECT ... )이 아니라, 일반 한국어 설명문이거나 변명(예: 'UPDATE 문은 규칙에 따라 생성할 수 없습니다')이라면,\n"
        "  절대 안전하다고 판단하지 말고 반드시 'safe': false 로 판정해.\n"
        "  그리고 'reason'에는 '유효한 SQL 쿼리 형태가 아닌 일반 텍스트 문장이 입력되어 차단되었습니다.'라고 작성해줘.\n\n"
...
def run_visualization_agent(data_sample_str: str, sql_query: str) -> dict:
    ...
    viz_system = (
        ...
        '  },\n'
        '  "insight": "분석 결과의 핵심 인사이트를 2~3문장으로 작성. **굵게** 강조 마크다운 사용 가능. 수치를 직접 언급하며 비즈니스 시사점까지 포함할 것"\n'
        "}"
    )
